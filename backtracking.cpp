...
void update_p(int val, int idx) { p[val][idx] = 1; }
void update_h(int val, int idx) { h[val][idx] = 1; }
void restore_p(int val, int idx) { p[val][idx] = 0; }
void restore_h(int val, int idx) { h[val][idx] = 0; }
int calc_score() {
  int sum = 0;
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < M; j++) {
